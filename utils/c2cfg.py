...
def build_annotation_blocktype(nodes, tampipe, linstructions, lmatrixcost):
	previous = None
	lasmcode = []
	for current in nodes:
		get_annotation(previous, current)
		get_blocktype(current)
		asm_code = current.assembly
		if (len(asm_code) > 0):
			inewblock = len(lasmcode)
			linst = asm_code.split()
			for e in linst:
				lasmcode.append(e)
				current.wcec = cal_wcec_pipeline(lasmcode, inewblock, wcec_table, tampipe) # tampipe: tamanho do pipeline
		else: current.wcec = 0
		current.energy = cal_energy (asm_code.split(), wcec_table) + cal_insteffect_energy(asm_code.split(), linstructions, lmatrixcost)
		if (current.tipo_estrutura == "2"):
			current.energy = current.energy - 4.68
			if (previous != None):
				previous.energy = previous.energy + 4.51
		previous = current
	...
	while (linha < len(string)):
		...
		if (not(string[linha] == "\n")):
			...
			if (not (lista[1] == "")):
				...
				#	for e in linst:
				#		lasmcode.append(e)
				#	bloco_atual.wcec = cal_wcec_pipeline(lasmcode, inewblock, wcec_table, tampipe) # tampipe: tamanho do pipeline
				#else: bloco_atual.wcec = 0
				#bloco_atual.energy = cal_energy (asm_code.split(), wcec_table)
...
def func_fim(pilha,indice,lista):
	...
	if ((pilha[-1])[0] == "inicio"):
		...
		if len(pilha) > 1 and pilha[-1][0] == "if" and pilha[-2][0] == "else":
			arestas = [indice,indice+1,indice-2,indice+1]
			pilha.pop()
			return [pilha,arestas]
	try:
		if ((pilha[-1])[0] == "if"): 
			
			if lista[indice+1].tipo.find("else") >= 0:
				arestas = [(pilha[-1])[1],indice+1] 
				pilha.pop()
				print arestas 
				return [pilha,arestas]
			pilha.pop() 
			arestas = [(pilha[-1])[1],indice+1,indice,indice+1]
			
			if len(pilha) > 0 and lista[indice+1].tipo.find("if") >= 0 and lista[indice+1].tipo.find("else") == -1:
				arestas = [(pilha[-1])[1],indice+1,indice,indice+1]
				print indice+1
...
def build_func(blocos,arestas):
	...
	#quantidade de blocos novos gerados para cada funcao
	indice = 0
	quant_blocos = []
	while indice < len(lista_funcoes):
		quant_blocos.append(0)
		indice += 1
						...
						if linhas[count3].find(lista_funcoes[count2] + " ") > 0 or linhas[count3].find(lista_funcoes[count2] + "(") > 0:
							...
							if count3 == 0:
								...
								if len(linhas) > 1:
									...
									for arest in arestas[indice][0]:
										if arest[0] == pos_bloco-1:
											arest[0] = pos_bloco
							...
							#caso nao seja a ultima linha do bloco, a chamada de funcao
								if count3 !=  len (linhas):
									...
									quant_blocos[indice] += 2
									
									# criacao do bloco com a funcao
									novo_bloco = bloco()
									...
									tipo = blocos[indice][count].tipo.split()
									if len(tipo) > 1:
										novo_bloco.tipo = tipo[-1]
									else:	
										novo_bloco.tipo = tipo[-1]
									...
									blocos[indice][count].tipo = tipo[0]
	...
	print "BLOCOS COM FUNCAO:"
	print blocos_with_func
	lista = build_func_aux(blocos,arestas,blocos_with_func,indice,new_blocos,new_arestas,0,quant_blocos)
...
def build_func_aux(blocos,arestas,blocos_with_func,indice,new_blocos,new_arestas,retorno,quant_novos_blocos):
	tuplas_funcao_atual = []
	...
	for tupla in tuplas_funcao_atual:
		...
		print len(new_blocos)
		print quant_novos_blocos[indice]
		new_arestas.append([len(new_blocos)-1-quant_novos_blocos[indice],tupla[2]+retorno])



		"""	
		cont2 = 0
		while cont2 < len(lista_funcoes):
			quant_novos_blocos[cont2] += quant_novos_blocos[indice]
			cont2 += 1"""

		

	
		lista = build_func_aux(blocos,arestas,blocos_with_func,indice,new_blocos,new_arestas,retorno2,quant_novos_blocos)
		new_arestas = lista[0]
		new_blocos = lista[1]
		cont += 1
...
def build_powerC_tab(lines):
	...
	for line in lines:
		if len(line.strip()) > 0:
			elem = line.split()	
			tabC[elem[0]] = [int(elem[1]), float(elem[2])]
...
def buildProbabilityBranch(root, current_block, clauses_if, strprob, powerC):
	...
	powerC_model = build_powerC_tab(abre_arquivo(powerC))
	for prob in probs:
		node_cond = root.createElement("condition")
		node_cond.setAttribute("id", clauses_if[i][1])
		# node_cond.setAttribute("express", clauses_if[i][0]) imprime no xml a expressao da condicao, i.e, V==V ou V==K etc.
		node_cond.setAttribute("clock-cycles",  str(powerC_model[clauses_if[i][0]][0])) #"5")
		node_cond.setAttribute("energy", str(powerC_model[clauses_if[i][0]][1])) # "5") # 
		...
		# cycles = cycles + powerC_model[clauses_if[i][0]][0]
		# energy = energy + powerC_model[clauses_if[i][0]][1]
		i = i+1
...
def buildProbabilityLoop(root, current_block, clauses_loop, strprob, powerC):
	...
	if (axprob.find("@LoopMaxProb") >= 0):
		...
		for prob in probs:
			...
			node_cond.setAttribute("clock-cycles", str(powerC_model[clauses_loop[i][0]][0])) # "5")
			node_cond.setAttribute("energy", str(powerC_model[clauses_loop[i][0]][1])) # "5")
			node_prob = root.createElement("probability")
...
def passa_graphml(edges,nodes, powerC):
	...
	for node in nodes:
		...
		bloco.setAttribute("energy", str(node.energy))
		...
		if (len(node.prob) > 0):
			...
			if (node.tipo_estrutura == "1"):
				...
				cycles_energy = buildProbabilityBranch(doc, bloco, conditions, strprob, powerC)
				bloco.setAttribute("energy", "0.0") #str(node.energy - cycles_energy[1])) # subtrai do bloco a qtde de ciclos e de energia de cada condicao
				bloco.setAttribute("clock-cycles", "0") #str(node.wcec - cycles_energy[0]))							
				...
				if (node.c.find("while") > 0):
					...
					reverse_express = converterPosFixa(conditions[len(conditions)-1])
					bloco.setAttribute("bool-express", reverse_express)					
				buildProbabilityLoop(doc, bloco, conditions, strprob, powerC)				
...
lista_funcoes = []
lista_tipos = ["void","int","float","double","char","long", "static", "unsigned","short"]
...
def insere_novos_tipos():
	ent = raw_input("Digite as novas estruturas declaradas no codigo C (de um espaco entre cada uma):\n")
	try:
		ent = ent.split()
		for palavra in ent:
			lista_tipos.append(palavra)
	except:
		pass
...
if __name__ == '__main__':
	...
	instrucoes = buildGroupsInstructions(argv[7])
	matrizcusto = buildMatrizCost(argv[7])
...
#	print afc_dict
	...
	get_all_blocks(linhas, int(argv[6]))
	...
	build_annotation_blocktype(lista[1], int(argv[6]), instrucoes, matrizcusto)
	#// calcula o wcec conforme o tamanho do pipeline
